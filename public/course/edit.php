...
// Recuperar los datos del curso a editar
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verificar el valor de $estado antes de actualizarlo
    ...
    $stmt = $conn->prepare("UPDATE courses SET title = ?, description = ?, status = ?, user_id = ? WHERE id = ?");
    $stmt->bind_param("sssii", $titulo, $descripcion, $estado, $user_id, $course_id);

    if ($stmt->execute()) {
        // Redirigir a dashboard.php después de la actualización exitosa
            ...
            <form action="" method="post">
                <table>
                    <tr>
                        ...
                        <td><input type="text" id="titulo" name="titulo" value="<?php echo htmlspecialchars($course['title']); ?>" required></td>
                    </tr>
                    <tr>
                        <th><label for="descripcion">Descripción:</label></th>
                        <td><textarea id="descripcion" name="descripcion" required><?php echo htmlspecialchars($course['description']); ?></textarea></td>
                    </tr>
                    <tr>
                        <th><label for="estado">Estado:</label></th>
                        <td>
                            <select id="estado" name="estado" required>
                                <option value="active" <?php if ($course['status'] == 'active') echo 'selected'; ?>>Activo</option>
                                <option value="inactive" <?php if ($course['status'] == 'inactive') echo 'selected'; ?>>Inactivo</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                            ...
                            <select id="user_id" name="user_id" required>
                                ...
                                if (!$result) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        $selected = ($row['id'] == $course['user_id']) ? 'selected' : '';
